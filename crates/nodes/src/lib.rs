...
impl Context {
    ...
    pub fn show<'a>(
...

        {
            ...
            );
            {
                ...
                ui.set_clip_rect(
                    self.canvas_rect_screen_space
                        .intersect(ui.ctx().input().screen_rect()),
                );
            ...
            );
            {
                ...
                let alt_mouse_clicked = self.io.emulate_three_button_mouse.is_active(&io.modifiers)
                    || self
                        .io
                        .alt_mouse_button
                        .map_or(false, |x| io.pointer.button_down(x));
                ...
                self.link_detatch_with_modifier_click = self
                    .io
    ...
    pub fn get_selected_nodes(&self) -> Vec<usize> {
        self.selected_node_indices
            .iter()
            .map(|x| self.nodes.pool[*x].id)
            .collect()
    }

    pub fn get_selected_links(&self) -> Vec<usize> {
        self.selected_link_indices
            .iter()
            .map(|x| self.links.pool[*x].id)
            .collect()
    }
    ...
    pub fn link_created(&self) -> Option<(usize, usize, bool)> {
        if (self.element_state_change & ElementStateChange::LinkCreated as usize) != 0 {
            let (start_pin_id, end_pin_id) = {
                ...
                let end_pin = &self.pins.pool[self
                    .click_interaction_state
                    .link_creation
                    .end_pin_index
                    .unwrap()];
...
impl Context {
    ...
    fn add_attribute(
        ...
        if kind != AttributeType::None {
            ...
            self.style
                .format_pin(pin, args, self.current_attribute_flags);
        ...
        match self.node_indices_overlapping_with_mouse.len() {
            ...
            1 => {
                self.hovered_node_index
                    .replace(self.node_indices_overlapping_with_mouse[0]);
            }
            _ => {
...
impl Context {
    fn node_pool_update(&mut self) {
        ...
        for (i, (in_use, node)) in self
            .nodes
            .in_use
            .iter_mut()
            .zip(self.nodes.pool.iter_mut())
            .enumerate()
        {
            if *in_use {
